...
<body>
    ...
    <form action="updateuser" method="post" enctype="multipart/form-data">
        <div class="form-group">
            <label >编号:<span style="color:#3d30ff;">${user.id}</span><br></label>
        </div>
        <%--编号：<span style="color:red;">${user.id}</span><br>--%>
        <div class="form-group">
            <label >用户名:<%--<span>${user.username}</span>--%></label>
            <%--<label >${user.username}</label>--%>
            <input  class="form-control" readonly="readonly" type="label" name="username"  placeholder="Email" value="${user.username}">
        </div>

        <div class="form-group">
            <label >密码:<%--<span></span>${user.password}--%></label>
            <%--<label >${user.password}</label>--%>
            <input  class="form-control" readonly="readonly" name="password" placeholder="Email" value="${user.password}">
        </div>
